...
import type { PathRefusal } from "../../../../desktop/src/generated/bindings";
...
export function EditorView() {
  // The route's two strings arrive as search params, which is what makes them survive their own
  // contents: a path holds `/`, `&` and `#` routinely, and `URLSearchParams` decodes exactly what
  // `editorPath` encoded.
  const [params] = useSearchParams();
  const repo = params.get("repo");
  const file = params.get("file");

  // Both or neither: an editor with no file is not a state this app has, and falling back to an
  // empty buffer would hide a malformed link instead of showing the list.
  if (repo === null || file === null || repo === "" || file === "") {
    return <Navigate to={runsPath()} replace />;
  }
  return <OpenedFile repo={repo} file={file} />;
}

function OpenedFile({ repo, file }: { repo: string; file: string }) {
  ...
  return (
    <main>
      ...
      {/* Every other view has one, and the subtree's route-change effect focuses it so a keyboard
      ...
      {opened.isLoading && <p className="dim">opening {file}…</p>}
