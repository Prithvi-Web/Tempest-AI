...
export function RunView({ id }: { id: number }) {
  ...
  // (TempestViews.tsx) with useGetRun's slow refetch as the fallback — this view owns no timer.

  if (run.isLoading) return <p className="dim">loading run #{id}…</p>;
  ...
  return (
    <main>
      <nav className="crumbs">
        <Link to={runsPath()}>Runs</Link> / #{data.id}
      </nav>
            ...
            <tbody>
              {unproven.map((t) => (
                <tr
                  ...
                  className="rowlink"
                  tabIndex={0}
                  onClick={() => navigate(targetPath(t.id))}
                  onKeyDown={(e) => e.key === "Enter" && navigate(targetPath(t.id))}
                >
                  <td>
                    {t.module}.{t.qualname}
                  </td>
