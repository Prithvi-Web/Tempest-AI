...
| **11 Local-First Data & Perf** | ✅ COMPLETE 2026-08-14 (soak PASS −3.7%/937 proves/0 fail) | ADR-0016..0019: versioned WAL store (refuse-newer proven byte-identical); content-addressed bundle store + budget GC; FTS5 search + `.tempest` export/import (byte-identical, L7); `bench` CI job + `bench_guard: PASS` (cold 0.297 s, 10k list 1.06 ms, 5 MB 23.9 ms, idle 112 MB/0%); L11 cancel <2 s + battery/thermal pause; 8-h soak → `bench/soak.json` |
