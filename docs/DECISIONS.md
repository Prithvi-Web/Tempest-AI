...
> **v3 note (2026-08-21).** All six refusals above were overturned by owner decision at the v3
> convergence — each with the ADR this paragraph demands, none by drift: image generation →
> ADR-0063 · TTS/STT → ADR-0065 · agent marketplace → ADR-0066 (**partial**: mandatory capability
> signing is retained, because the supply-chain threat named above is real and survives the
> change of mandate) · chat as the primary surface → ADR-0067 · MongoDB → ADR-0068 (the data
> model and wire protocol, never the SSPL binary) · general-assistant framing → ADR-0069.
> The refusal table is retained verbatim, deliberately: a future contributor must be able to read
> what was refused, why, and why that changed. See `docs/TEMPEST-V3-MASTER-PROMPT.md` §5.1.
...
## v3 Convergence decisions — ADR-0063 … ADR-0076 (2026-08-21)

> Landed at Phase C0 from `docs/DECISIONS-V3-ADDENDUM.md`, the v3 master prompt's drafted
> decisions (`docs/TEMPEST-V3-MASTER-PROMPT.md`), reviewed against the tree before landing.
> Six of these overturn refusals recorded in ADR-0038. Per that ADR's own instruction —
> *"overturning any refusal above takes an ADR, not a drift"* — each engages with the original
> reason rather than ignoring it. ADR-0038's refusal table is retained verbatim above and
> amended with a pointer, never deleted.
