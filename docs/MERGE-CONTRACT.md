...
| `api/server/services/Agents/**` | **REPLACE** | ADR-0075. Thin client over boundary E to the Python orchestrator. The largest and most painful row in this table; accepted knowingly. *C5 COMPLETE (ADR-0078/ADR-0079): the desktop path's delegation landed as the HOST's protocol seam (`agent_chat.rs` → boundary A) — chat turns, agent CRUD, HITL resume and the steer family all answered there; tool-bearing turns dispatch through `run_task` (`agentturn.py`, gate_audit's sixth declared path); boundary E has no Node-initiated RPC and the vendored Express agent stack stays dormant, held by `runtime_check`. The deviation from this row's wording is recorded in ADR-0078; the back half's working decisions in ADR-0079.* |
