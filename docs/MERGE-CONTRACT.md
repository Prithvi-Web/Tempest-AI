...
| `api/server/services/Agents/**` | **REPLACE** | ADR-0075. Thin client over boundary E to the Python orchestrator. The largest and most painful row in this table; accepted knowingly. *C5 status (ADR-0078): the desktop path's delegation landed as the HOST's protocol seam (`agent_chat.rs` → boundary A → the engine's chat-turn service) — boundary E has no Node-initiated RPC and the vendored Express agent stack stays dormant, held by `runtime_check`; the deviation from this row's wording is recorded in ADR-0078.* |
